...
struct emdb_trie_node_s{
  union{ int64_t base : 48; int64_t value : 48; };   /** negative means prev empty index **/
  int64_t check:48;                               /** negative means next empty index **/
};
...
struct emdb_trie_s{
  ...
  int64_t              block_head_full : 48;
  int64_t              block_head_close: 48;
  int64_t              block_head_open : 48;
  int64_t              capacity        : 48;
  int64_t              size            : 48;
  int64_t              no_delete       : 48;
  int64_t              reject[257]; 
  size_t               tracking_node[EMDB_TRIE_NUM_TRACKING_NODES + 1];
};
