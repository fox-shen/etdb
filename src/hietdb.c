...
etdb_set(etdb_client_t *client, etdb_str_t *key, etdb_str_t *value)
{
  ...
  uint8_t *pos  = memcpyn(client->command_buf, "set ", sizeof("set ") - 1);
...
{
  etdb_status_t status = etdb_string("+ERROR");
  int len = key->len + sizeof("get ") - 1;

  CHECK_COMMAND_BUF

  uint8_t *pos = memcpyn(client->command_buf, "get ", sizeof("get ") - 1);
  pos          = memcpyn(pos, key->data, key->len);
...
etdb_status_t 
etdb_del(etdb_client_t *client, etdb_str_t *key)
{
  etdb_status_t status = etdb_string("+ERROR");
  int len = key->len + sizeof("del ") - 1;

  CHECK_COMMAND_BUF

  uint8_t *pos = memcpyn(client->command_buf, "del ", sizeof("del ") - 1);
  pos          = memcpyn(pos, key->data, key->len);
